...
extern void w2_aml_pci_rmmod(void);
...
static int amlbt_bind_bus(void)
{
    ...
    if (w2_g_aml_plat_pci->pci_dev == NULL)
    {
    ...
    else
    {
        if (p_pcie->link == NULL)
        {
            p_pcie->link = device_link_add(&amlbt_pcie_device.dev, &w2_g_aml_plat_pci->pci_dev->dev, DL_FLAG_PM_RUNTIME | DL_FLAG_STATELESS);
            if (p_pcie->link == NULL)
            {
...
static void amlbt_unbind_bus(void)
{
    ...
    if (p_pcie->link != NULL)
    {
        if (w2_g_aml_plat_pci->pci_dev && device_is_registered(&w2_g_aml_plat_pci->pci_dev->dev))
        {
            link = find_device_link(&amlbt_pcie_device.dev, &w2_g_aml_plat_pci->pci_dev->dev);
...
static void amlbt_pcie_register(void)
{
    ...
    else if (!w2_g_pci_driver_insmoded)
    {
